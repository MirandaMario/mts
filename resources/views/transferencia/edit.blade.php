...
@section('title','Edicion de Transferencia')
@section('contenido')
...
<div class="row" style=" {{ config('constantes.FONT') }}">
    ...
    {!!Form::model($transferencia,['method'=>'PATCH','route'=>['transferencia.update',$transferencia->id_transferencia], 'autocomplete'=>'off', 'id' =>'mf'])!!}
    {{Form::token()}}
    <div>
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 pa">
            <div class="panel panel-primary">
                <div class="panel-heading" style="font-size:125%; height: 40px;">
                    Tienda origen {{$transferencia->id_origen}} &nbsp;&nbsp;&nbsp;&nbsp;Destino
                    {{$transferencia->id_destino}} &nbsp;&nbsp;&nbsp;&nbsp; {{date('d-m-Y H:i',strtotime($transferencia->created_at))}}
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-sm-2 col-md-2 col-xs-12 pa">
                            <label for="cantidad" class="apa">
                                <a id="min" class="unselectable"><span class="fa fa-minus-square"
                                        style=" font-size: 18px;" aria-hidden="true"></span>
                                </a> &nbsp;&nbsp; Cant &nbsp;&nbsp;
                                <a id="plus" class="unselectable"><span id="plus" class="fa fa-plus-square"
                                        style=" font-size: 18px;" aria-hidden="true">
                                    </span></a></label>
                            <div class="form-group">
                                <input type="number" id="pcantidad" name="pcantidad"
                                    class="form-control input-sm text-center pa" value="1">
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12 pa ">
                            <div class="well">
                                <table>
                                    ...
                                    @php $t= 0; @endphp
                                    @foreach ($detalles as $det) @php $t += $det->cantidad; @endphp @endforeach
                                    <th width="10%">
                                        <h4 class="text-right" id="total">{{$t}}</h4>
                                    </th>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12" >
                            ...
                            <div class="form-group tetx-right">
                                ...
                                <input type="hidden" id="idtienda" name="idtienda"
                                    value="{{$transferencia->id_origen}}">
                                <input type="hidden" id="idtienda2" name="idtienda2"
                                    value="{{$transferencia->id_destino}}">
                    ...
                    <div>
                        <table id="detalles" class="table table-striped table-bordered table-condensed table-hover"
                            ...
                            <tr class="selected" id="fila{{$con}}">
                                ...
                                <td><input type="hidden" name="idarticulo[]" size="39"
                                        value="{{$det->idarticulo}}">{{$det->idarticulo}} {{$det->nombre}}  {{$det->nombreModelo}} {{$det->codigo}} {{$det->nombreMarca}}</td>
                                <td class="text-center"><input name="cantidad[]"  class="outlinenone"  value="{{$det->cantidad}}" readonly></td>
...
<script type="text/javascript">
    $("#transbtn").css({"color":"orange" , "font-size": "18px"});
        ...
        var total='<?php echo $t;?>' * 1;
        var articulos=<?php echo json_encode($cantidad);?>;
        console.log(articulos, cont, total);
